...
run_collect_case() {
  ...
  if _run_with_coverage "$cov_filter" "$in_candidate" "${args[@]}" >"$log_path" 2>&1; then
    ...
    echo "[pgo-gather] warning: $in_label finished without junit output; writing synthetic pass report" >&2
    write_synthetic_junit_report \
      "$report_path" \
      "$in_label" \
      "pass" \
      "$in_label exited 0 but did not emit JUnit; synthetic pass report generated by pgo-gather."
    return 0
  fi
  if [[ -f "$report_tmp" ]]; then
    mv -f "$report_tmp" "$report_path"
...
write_synthetic_junit_report() {
  ...
  escaped_suite="$(xml_escape_attr "$suite_name")"
  escaped_message="$(xml_escape_attr "$message")"
  if [[ "$status" != "pass" ]]; then
    failures="1"
  fi
  ...
  {
    printf '%s\n' '<?xml version="1.0" encoding="UTF-8"?>'
    printf '<testsuites tests="1" failures="%s" errors="0" skipped="0">\n' "$failures"
    printf '  <testsuite name="%s" tests="1" failures="%s" errors="0" skipped="0" time="0">\n' "$escaped_suite" "$failures"
    printf '    <testcase classname="%s" name="pgo-gather-run" time="0">' "$escaped_suite"
    if [[ "$failures" != "0" ]]; then
