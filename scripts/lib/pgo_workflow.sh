...
pgo_collect() {
    ...
    if [[ "$platform" == "Darwin" ]]; then
        ...
        rm -f "$INF_PGO_COLLECT_DIR"/*.profraw 2>/dev/null || true
    elif [[ "$compiler_id" == "GNU" ]]; then
        # GCC uses .gcda files
        rm -f "$INF_PGO_COLLECT_DIR"/*.gcda 2>/dev/null || true
    fi
    ...
    echo "[pgo_collect] Build complete. Profile data written to: $INF_PGO_COLLECT_DIR"
...
# =============================================================================
pgo_merge() {
    local output_file="${1:-$INF_PGO_MERGED_FILE}"
    local env_info
    local platform
    local compiler_id
    ...
    if [[ "$compiler_id" == "Clang" ]]; then
        ...
        while IFS= read -r -d '' file; do
            profraw_files+=("$file")
        done < <(find "$INF_PGO_COLLECT_DIR" -name "*.profraw" -type f -print0 2>/dev/null || true)

        if [[ ${#profraw_files[@]} -eq 0 ]]; then
            echo "[pgo_merge] WARNING: No .profraw files found in $INF_PGO_COLLECT_DIR" >&2
        ...
        while IFS= read -r -d '' file; do
            gcda_files+=("$file")
        done < <(find "$INF_PGO_COLLECT_DIR" -name "*.gcda" -type f -print0 2>/dev/null || true)

        if [[ ${#gcda_files[@]} -eq 0 ]]; then
            echo "[pgo_merge] WARNING: No .gcda files found in $INF_PGO_COLLECT_DIR" >&2
            echo "[pgo_merge] Did you run tests after pgo_collect?" >&2
            return 1
        fi
        ...
        while IFS= read -r -d '' file; do
            pgd_files+=("$file")
        done < <(find "$INF_PGO_COLLECT_DIR" -name "*.pgd" -type f -print0 2>/dev/null || true)

        if [[ ${#pgd_files[@]} -eq 0 ]]; then
            echo "[pgo_merge] WARNING: No .pgd files found in $INF_PGO_COLLECT_DIR" >&2
            echo "[pgo_merge] Did you run tests after pgo_collect?" >&2
            return 1
        fi

        echo "[pgo_merge] Found ${#pgd_files[@]} .pgd files"
        echo "[pgo_merge] MSVC profile data at: $INF_PGO_COLLECT_DIR"
...
pgo_use() {
    ...
    if [[ "$compiler_id" == "Clang" && -z "$profdata_file" ]]; then
        profdata_file="$INF_PGO_MERGED_FILE"
    fi
    ...
    (
        ...
        if [[ "$compiler_id" == "Clang" && -n "$profdata_file" && -f "$profdata_file" ]]; then
            export INF_PGO_PROFDATA="$profdata_file"
            cmake --preset "$preset" \
    ...
    echo "[pgo_use] Output directory: $INF_BUILD_ROOT/release (or equivalent)"
}
...
pgo_info() {
    ...
    echo "PGO Root:       $INF_PGO_ROOT"
    echo "Collect Dir:    $INF_PGO_COLLECT_DIR"
    echo "Profile Dir:    $INF_PGO_PROFILE_DIR"
    echo "Merged File:    $INF_PGO_MERGED_FILE"

    if [[ -d "$INF_PGO_COLLECT_DIR" ]]; then
        local count
        count=$(find "$INF_PGO_COLLECT_DIR" \( -name "*.profraw" -o -name "*.gcda" -o -name "*.pgd" \) 2>/dev/null | wc -l || echo "0")
...
_pgo_main() {
    ...
    case "$command" in
        ...
        help|--help|-h)
            ...
            echo "  INF_PGO_ROOT       Base directory for PGO data (default: build/pgo)"
            echo "  INF_BUILD_ROOT     Build root directory"
            echo "  INF_CPP_ROOT       C++ source root (CMakeLists.txt location)"
            echo ""
            echo "Workflow:"
